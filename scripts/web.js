...
function obtenerCantidadArticulos() {
    return localStorage.getItem('cantidadArticulos') || '0';
  }

function agregarAlCarrito() {
  cantidadArticulos++;
  localStorage.setItem('cantidadArticulos', cantidadArticulos.toString());
  actualizarIconoCarrito();
}

function actualizarIconoCarrito() {
  const iconoCarrito = document.getElementById('carrito-icon');
  const spanNumero = iconoCarrito.querySelector('.badge');
  spanNumero.textContent = cantidadArticulos;
}

document.addEventListener('DOMContentLoaded', function() {
  const botonAgregar = document.getElementById('aniadirCarrito');
